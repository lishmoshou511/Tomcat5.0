...
public class SocketInputStream extends InputStream {
	 ...
	 *                     the whole line.
	 */
	public void readHeader(org.apache.catalina.connector.comet.HttpHeader header)
			throws IOException {
		...
		while (!colon) {
			// if the buffer is full, extend it
			if (readCount >= maxRead) {
				if ((2 * maxRead) <= org.apache.catalina.connector.comet.HttpHeader.MAX_NAME_SIZE) {
					char[] newBuffer = new char[2 * maxRead];
		...
		while (validLine) {
			...
			while (!eol) {
				// if the buffer is full, extend it
				if (readCount >= maxRead) {
					if ((2 * maxRead) <= org.apache.catalina.connector.comet.HttpHeader.MAX_VALUE_SIZE) {
						char[] newBuffer = new char[2 * maxRead];
